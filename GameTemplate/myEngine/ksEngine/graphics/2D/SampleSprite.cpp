...
void SampleSprite::InitVertexBuffer(float w, float h)
{
...
void SampleSprite::Update(const CVector3& trans, const CQuaternion& rot, const CVector3& scale, CVector2 pivot)
{
...
void SampleSprite::InitIndexBuffer()
{
	//これがインデックス。頂点番号。
	int index[6] = {
		0,1,2,		//三角形一つ目
...
void SampleSprite::InitConstantBuffer()
{
...
void SampleSprite::InitSamplerState()
{
...
void SampleSprite::Init(const wchar_t* texFilePath, float w, float h)
{
	m_size.x = w;
	m_size.y = h;
	//頂点バッファの初期化。
	...
	InitSamplerState();
	//シェーダーのロード。
	m_vs.Load("Assets/shader/SampleSprite.fx", "VSMain", ksEngine::Shader::EnType::VS);
	m_ps.Load("Assets/shader/SampleSprite.fx", "PSMain", ksEngine::Shader::EnType::PS);
