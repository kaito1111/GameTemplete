...
void ShadowMap::RenderToShadowMap()
{
	...
	for (auto& caster : m_shadowCasters) {
		caster->SetRenderMode(enShadowDraw);
		caster->Draw(
			m_lightViewMatrix,
			m_lightProjMatrix
		);
...
void ShadowMap::SaveRenderTarget()
{
	auto d3dDeviceContext = g_graphicsEngine->GetD3DDeviceContext();
	...
	d3dDeviceContext->RSGetViewports(&m_numViewport, &m_oldViewports);
}

void ShadowMap::BackUpRenderTarget()
{
	auto d3dDeviceContext = g_graphicsEngine->GetD3DDeviceContext();
	//元に戻す。
	...
	d3dDeviceContext->RSSetViewports(m_numViewport, &m_oldViewports);
	//レンダリングターゲットとデプスステンシルの参照カウンタを下す。
	if (m_oldRenderTargetView != nullptr) {
		m_oldRenderTargetView->Release();
		m_oldRenderTargetView = nullptr;
	}
	if (m_oldDepthStencilView != nullptr) {
